...
def test_pty_tab_count_endpoint_reflects_resident_engines(client, monkeypatch):
    ...
    just surfaces `session_runner.count_resident_engines()`, plus (issue
    #140) the additive `"engines"` listing from
    `session_runner.list_live_engines`."""
    ...
    monkeypatch.setitem(session_runner._stream_json_engines, 1, _FakeEngine())
    data = client.get("/api/pty-tabs/count").json()
    assert data["count"] == 1
    assert data["engines"] == [{"card_id": 1, "model": "claude-sonnet-5", "effort": "auto"}]

    monkeypatch.setitem(session_runner._stream_json_engines, 2, _FakeEngine())
    data = client.get("/api/pty-tabs/count").json()
    assert data["count"] == 2
