...
**Never spawn a subagent.** No part of this skill — including issue implementation, Phase 3's test-fixing, and any exploration needed to diagnose a failure — may dispatch a subagent (via the Agent tool, a fork, or any other delegation mechanism) or run anything in parallel. Everything happens directly, one thing at a time, in this same continuous session. (Phase 2 below restates this for issue implementation specifically, along with the rate-limit-budget rationale for that case.)
